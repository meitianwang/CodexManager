...
function Add-SmokeResultChecks {
  ...
  foreach ($page in @("accounts", "proxy", "settings")) {
    ...
    if ($page -eq "accounts") {
      ...
      $pagePassed = $pagePassed -and
        ...
        (Get-ArrayCount (Get-PropertyValue $accounts "viewControlButtons")) -eq 2 -and
        (Test-ContainsAll (Get-PropertyValue $accounts "viewControlButtons") @("Grid view", "List view")) -and
        (Test-ContainsAll (Get-PropertyValue $accounts "actionAccessibilityLabels") @("Switch to this", "Refresh usage", "Delete")) -and
  ...
  Add-Check "automated.accountWorkflows" (
    ...
    (Get-StringValue (Get-PropertyValue $accountWorkflows "importPackageFileKind")) -eq "package" -and
    (Get-NumberValue (Get-PropertyValue $accountWorkflows "importPackageDraftAccountCount")) -eq 1 -and
    (Get-StringValue (Get-PropertyValue $accountWorkflows "importPackageDraftAccountId")) -eq "acct-package" -and
