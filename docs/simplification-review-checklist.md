# CodexManager Simplification Review Checklist
...
- [x] 删除 `StoreFileRepository.loadStore()` 里的“截取第一个 JSON 对象继续恢复”逻辑，坏文件直接备份并重置，或者直接失败。
  Evidence: `Sources/CodexManager/Infrastructure/StoreFileRepository.swift:27-40`, `Sources/CodexManager/Infrastructure/StoreFileRepository.swift:101-154`
...
- [x] 删除 `CloudKitAccountsSyncService.pullRemoteAccountsIfNeeded()` 在远端 payload 无效时把本地快照重新推回远端的逻辑。
  Evidence: `Sources/CodexManager/Infrastructure/CloudKitAccountsSyncService.swift:134-139`, `Sources/CodexManager/Infrastructure/CloudKitAccountsSyncService.swift:268-282`
  Reason: 这属于“远端异常时自动补写”的 fallback；在测试环境和单人项目里，显式暴露问题比自动补救更清晰。

- [x] 梳理 proxy runtime 中对请求内容的隐式修正，优先移除“自动改写后重试”这类兼容逻辑。
  Evidence: `Sources/CodexManager/Infrastructure/SwiftNativeProxyRuntimeService.swift:319-324`, `Sources/CodexManager/Infrastructure/SwiftNativeProxyRuntimeService.swift:378-415`
...
- [x] 收缩 `Domain/Protocols.swift` 的协议层，只保留确实存在多个实现或确实需要稳定替身边界的协议。
  Evidence: `Sources/CodexManager/Domain/Protocols.swift:3-158`
...
- [x] 优先把只服务于测试注入的轻量依赖改成更直接的 concrete type 或 closure 注入，避免“一个类型配一个 protocol”。
  Evidence: `Sources/CodexManager/Domain/Protocols.swift:19-137`, `Sources/CodexManager/App/AppContainer.swift:71-178`
  Reason: 这个项目只有你自己使用，过多接口层会显著提高理解成本。

- [x] 重新审视 `AppContainer.liveOrCrash()` 里重复构建的第二套 proxy 依赖图，能共用就共用，不能共用就把理由写清楚。
  Evidence: `Sources/CodexManager/App/AppContainer.swift:20-46`, `Sources/CodexManager/App/AppContainer.swift:110-126`
...
- [x] 拆分 `SwiftNativeProxyRuntimeService.swift`，至少拆成“HTTP surface / request normalization / upstream transport / model mapping”几块。
  Evidence: 文件总长 1666 行；入口和传输逻辑集中在 `Sources/CodexManager/Infrastructure/SwiftNativeProxyRuntimeService.swift:3-415`
  Reason: 这个文件已经大到无法低成本确认改动影响，继续叠逻辑只会越来越难维护。

- [x] 拆分 `SectionCard.swift`，把基础容器、按钮样式、surface modifier、progress 组件分文件。
  Evidence: 文件总长 1265 行；基础卡片和大量视觉 primitive 混在 `Sources/CodexManager/UI/SectionCard.swift:6-320`
...
- [x] 拆分 `CloudKitAccountsSyncService.swift`，至少把 availability、accounts sync、current selection sync 分开。
  Evidence: `Sources/CodexManager/Infrastructure/CloudKitAccountsSyncService.swift:8-49`, `Sources/CodexManager/Infrastructure/CloudKitAccountsSyncService.swift:51-360`
...
- [x] 精简 proxy runtime 里的模型兼容映射，只保留你当前真实会用到的模型名。
  Evidence: `Sources/CodexManager/Infrastructure/SwiftNativeProxyRuntimeService.swift:9-32`
...
- [x] 评估是否还需要 AppSettings / AccountsStore 中的 legacy decode 与 fallback 字段解析；确认可抛弃后就删掉。
  Evidence: `Sources/CodexManager/Domain/Models.swift` 中存在多处 legacy / fallback 解析分支；测试也仍覆盖 legacy 行为，例如 `Tests/CodexManagerTests/AppSettingsCodableTests.swift`、`Tests/CodexManagerTests/StoreFileRepositoryTests.swift`
  Reason: 你的约束已经明确允许破坏性更新，不必长期背着历史格式迁移代码。

- [x] 对视觉层做一次平台收敛，停止在同一组件里同时维护 glass / material / fallback 多套视觉路径。
  Evidence: `Sources/CodexManager/UI/SectionCard.swift:153-207`, `Sources/CodexManager/UI/SectionCard.swift:223-265`
...
- [x] 清理 `SwiftNativeProxyRuntimeService.status()` 里大量 `try?` 式静默失败，明确哪些状态读取失败应该直接暴露错误。
  Evidence: `Sources/CodexManager/Infrastructure/SwiftNativeProxyRuntimeService.swift:59-73`
  Reason: `apiKey` 和 `availableAccounts` 读取失败现在直接吞掉，界面看到的是“正常但为空”，这不利于定位真实问题。

- [x] 清理启动期“失败后忽略”的异步任务，尤其是 launch-at-startup 同步。
  Evidence: `Sources/CodexManager/App/AppContainer.swift:152-158`
