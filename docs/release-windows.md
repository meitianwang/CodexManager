...
The smoke test waits for the process to exit, checks its result JSON, and verifies that the real packaged app starts, exposes the preload IPC bridge, renders and screenshots the Accounts, Proxy, and Settings pages, records screenshot dimensions and byte sizes, imports the current auth, imports and exports an account transfer package, smart-switches to the best seeded account, restores the seeded current account, writes `accounts.json`, `settings.json`, and `.codex\auth.json` to disk, persists settings, starts the local proxy, passes `/health`, rejects an unauthenticated proxy request, and exits cleanly.
