...
describe("Codex upstream client", () => {
  it("uses the injected desktop platform user agent by default", async () => {
    ...
    const client = new CodexUpstreamClient(
      ...
      { userAgent: "codex_cli_rs/0.116.0 (macOS; arm64) CodexManager/0.1" }
    );
    ...
    expect(sentHeaders?.get("User-Agent")).toBe("codex_cli_rs/0.116.0 (macOS; arm64) CodexManager/0.1");
  });
  ...
  it("preserves a caller-provided user agent header", async () => {
    ...
    expect(sentHeaders?.get("User-Agent")).toBe("custom-client/1.0");
  });
